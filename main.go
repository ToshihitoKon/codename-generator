...
func main() {
	...
	// log.Printf("readBytes: %d", readBytes)
	// fmt.Printf("randSource: %d", randSource)

	r := rand.New(rand.NewSource(randSourceInt64))
	fmt.Printf(TeamName(r))
}

// https://github.com/iwasa-kosui/isucon-team-name
// Copyright (c) 2020 ISUCON
func TeamName(r *rand.Rand) string {
	return generatePrefix(r) + generateMiddle(r) + generateSuffix(r)
}
...
func generatePrefix(r *rand.Rand) string {
	if r.Intn(2) < 1 {
		return prefixData[r.Intn(len(prefixData))]
	} else {
		return ""
	}
}

func generateMiddle(r *rand.Rand) string {
	return middleData[r.Intn(len(middleData))]
}

func generateSuffix(r *rand.Rand) string {
	return suffixData[r.Intn(len(suffixData))]
}
